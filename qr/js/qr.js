...
async function enviarDatosBackend(datos) {
    try {
        ...
        // Siempre detenemos la cámara al recibir respuesta para ahorrar batería
        if (typeof detenerCamara === "function") {
            await detenerCamara(); 
        }

        // Convertimos la respuesta a JSON para leer el campo "mensaje"
        ...
        if (response.ok) {
            ...
            // Aquí capturamos el mensaje específico que enviamos desde el Backend
            // Puede ser: "No se puede escanear: El ticket expiró (más de 24h)."
